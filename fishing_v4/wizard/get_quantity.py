...
class GetQuantity(models.TransientModel):
    ...
    reception_id = fields.Many2one('fishing.reception', string="Reception",
                                   default=lambda self: self._context['reception_id'])
    ...
    def save_products_to_packaging(self):
        ...
        new_pallet = self.env['stock.service.pallet'].create(
            ...
            'stock.pallet'].create({'location_id': stock_location.id, 'reception_id': self.reception_id.id})
        ...
        for p in self.product_ids:
            ...
            if quantity_data.type == 'service':
                ...
                new_box = {
                    ...
                    'product_id': p.product_id.id,
                    'reception_id': self.reception_id.id or False,
                    'quantity': p.qte,
                }
                box_create = self.env["fish.service.stock.caisse"].create(new_box)
                if self.internal_packaging:
