...
class Da(models.Model):
    ...
    packages_count = fields.Integer(string="Package Count", compute='_compute_packages_count')
    treatment_count = fields.Integer(string="Treatment Count", compute='_compute_treatment_count')
    pack_service_count = fields.Integer(string="Treatment Count", compute='_compute_pack_service_count')
    tunnel_count = fields.Integer(string="Tunnel Count", compute='_compute_tunnel_count')
    ...
    # 4i function tlewed l 4ak id_reception li ma36ilhe 4ak lwa9t (self.id) t5aras houwe mewjoud kem men
    # mara v stock.quant.packages
    def _compute_packages_count(self):
        packages_count = self.env['stock.quant.package'].search_count([('reception_id', '=', self.id)])
        self.packages_count = packages_count

    def _compute_treatment_count(self):
        treatment_count = self.env['fishing.reception.detail'].search_count([('reception_id', '=', self.id)])
        self.treatment_count = treatment_count

    def _compute_pack_service_count(self):
        pack_service_count = self.env['fishing.reception.detail'].search_count([('reception_id', '=', self.id)])
        self.pack_service_count = pack_service_count

    def _compute_tunnel_count(self):
        tunnel_count = self.env['fishing.reception.detail'].search_count(
            [('reception_id', '=', self.id), ('will_be_tunneled', '=', True), ('status', 'in', ['2', '3'])])
